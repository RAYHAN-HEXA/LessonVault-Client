...
// ReportsModal component - moved outside to avoid recreation on render
const ReportsModal = ({ caseData, onClose, requestIgnore, requestDelete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[2rem] w-full max-w-2xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="bg-[#1F4D2B] p-6 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4 text-white">
            <div className="p-2 bg-white/10 rounded-lg">
              <ShieldAlert size={24} className="text-[#C9D8C5]" />
            </div>
            <div>
        ...
        <div className="p-8 overflow-y-auto">
          <div className="mb-8">
            <span className="text-xs font-bold text-[#8A8F98] uppercase tracking-widest block mb-2">
              Flagged Content
            </span>
            <h4 className="text-2xl font-bold text-[#1F4D2B] font-serif leading-tight">
              "{caseData.title}"
            </h4>
          </div>
          ...
          <div className="space-y-4">
            ...
            {caseData.reports.map((report) => (
              <div
                key={report._id}
                className="p-5 rounded-2xl bg-[#EEF6EF] border border-[#E5ECE2]"
              >
                <div className="flex items-center justify-between mb-3 pb-3 border-b border-[#E5ECE2]/50">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[#1F4D2B]/10 flex items-center justify-center">
                      <User size={12} className="text-[#1F4D2B]" />
                    </div>
                    <span className="text-sm font-bold text-[#1F4D2B]">
        ...
        {/* Footer Actions */}
        <div className="p-6 border-t border-[#E5ECE2] flex flex-col sm:flex-row justify-end gap-3 bg-[#F8FAF6] shrink-0">
          <button
            onClick={() => requestIgnore(caseData)}
            className="px-6 py-3 rounded-xl border border-[#C9D8C5] text-[#6B7280] font-bold hover:bg-[#EEF6EF] transition-colors text-sm flex items-center justify-center gap-2"
          >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm animate-fade-in-up border border-[#E5ECE2]">
        <div
          className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${
            isDelete ? "bg-red-100 text-red-500" : "bg-[#EEF6EF] text-[#6B7280]"
          }`}
        >
      ...
      >
        {loading ? (
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="border-b border-[#E5ECE2] bg-[#F8FAF6]">
                  ...
                  <th className="p-6 text-xs font-bold text-[#8A8F98] uppercase tracking-widest w-2/12 text-center">
                    Severity
                  </th>
                  ...
                  >
                    ...
                    <td className="p-6 pl-10">
                      <div className="flex items-center gap-4">
                        <div className="p-2.5 rounded-xl bg-[#EEF6EF] text-[#1F4D2B] group-hover:bg-white transition-colors">
                          <FileText size={20} />
                        </div>
                        <div>
                          <span className="font-serif font-bold text-[#1F4D2B] text-lg leading-tight line-clamp-1 block mb-1 max-w-xs">
                            {group.title}
                          </span>
                          <span className="text-[10px] text-[#8A8F98] font-mono bg-[#F8FAF6] px-1.5 py-0.5 rounded border border-[#E5ECE2]">
                            ID: {group.postId}
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="p-6 pr-10 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-white border border-[#E5ECE2] text-[#1F4D2B] hover:bg-[#1F4D2B] hover:text-white transition-colors text-xs font-bold uppercase tracking-wider flex items-center gap-2 shadow-sm group/btn"
                        >
                        ...
                        <button
                          ...
                          className="p-2 rounded-full text-[#8A8F98] hover:text-[#6E9277] hover:bg-[#6E9277]/10 transition-colors"
                        ...
                        <button
                          ...
                          className="p-2 rounded-full text-[#8A8F98] hover:text-red-500 hover:bg-red-50 transition-colors"
                          title="Delete Lesson"
                        >
                          <Trash2 size={18} />
                        </button>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-24 h-24 rounded-full bg-[#6E9277]/10 flex items-center justify-center mb-6 text-[#6E9277] animate-pulse">
              <CheckCircle size={40} />
            </div>
